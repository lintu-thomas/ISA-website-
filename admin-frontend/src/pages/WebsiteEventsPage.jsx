                  ...
                  <tr key={event._id} className="border-b border-sju-border/50 hover:bg-[#FAF8F3] transition-colors group">
                    <td className="p-5 font-bold text-sju-navy text-[16px]">
                      <div className="flex items-center gap-3">
                        {event.imageUrl ? (
                           <img src={`https://isa-backend-26le.onrender.com${event.imageUrl}`} alt={event.title} className="w-10 h-10 rounded-[4px] object-cover border border-sju-border shadow-sm" onError={(e) => { e.target.src = 'https://via.placeholder.com/40x40?text=Event'; }} />
                        ) : (
